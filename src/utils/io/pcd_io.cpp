 ...
 * Open3D is open source under the MIT license:
...
bool ReadPCDData(FILE* file, const PCDHeader& header, std::vector<Eigen::Vector3f>& pointcloud) {
    ...
    if (header.datatype == PCD_DATA_ASCII) {
        ...
        while (fgets(line_buffer, DEFAULT_IO_BUFFER_SIZE, file) && idx < header.points) {
            ...
            for (const auto& field: header.fields) {
                if (field.name == "x") {
                    pointcloud[idx].x() = UnpackASCIIPCDElement(
                        strs[field.count_offset].c_str(),
                        field.type,
                        field.size
                    );
                } else if (field.name == "y") {
                    pointcloud[idx].y() = UnpackASCIIPCDElement(
                        strs[field.count_offset].c_str(),
                        field.type,
                        field.size
                    );
                } else if (field.name == "z") {
                    pointcloud[idx].z() = UnpackASCIIPCDElement(
                        strs[field.count_offset].c_str(),
                        field.type,
                        field.size
                    );
        ...
        for (const auto& field: header.fields) {
            ...
            if (field.name == "x") {
                for (int i = 0; i < header.points; i++) {
                    pointcloud[i].x() = UnpackBinaryPCDElement(
                        base_ptr + i * field.size * field.count,
                        field.type,
                        field.size
                    );
                }
                ...
                for (int i = 0; i < header.points; i++) {
                    pointcloud[i].y() = UnpackBinaryPCDElement(
                        base_ptr + i * field.size * field.count,
                        field.type,
                        field.size
                    );
                ...
                for (int i = 0; i < header.points; i++) {
                    pointcloud[i].z() = UnpackBinaryPCDElement(
                        base_ptr + i * field.size * field.count,
                        field.type,
                        field.size
                    );
...
bool write_pcd(
    ...
    PCDHeader header;
    if (!GenerateHeader(pointcloud, bool(params.write_ascii), bool(params.compressed), header)) {
        return false;
    }
