...
inline __attribute__((always_inline)) void SmallIVox::get_closest_point(
    ...
    {
        iter = grids_map.find(
            (hash_key & 0xFFFFFFFFFFFF0000) | ((hash_key + 1L) & 0x000000000000FFFF)
        );
        if (iter != grids_map.end()) {
            closest_pt.push_back(*iter->second);
        }
    ...
    {
        iter = grids_map.find(
            (hash_key & 0xFFFFFFFF0000FFFF) | ((hash_key + (1L << 16)) & 0x00000000FFFF0000)
        );
        if (iter != grids_map.end()) {
            closest_pt.push_back(*iter->second);
        }
    ...
    {
        iter = grids_map.find(
            (hash_key & 0xFFFF0000FFFFFFFF) | ((hash_key + (1L << 32)) & 0x0000FFFF00000000)
        );
        if (iter != grids_map.end()) {
            closest_pt.push_back(*iter->second);
        }
